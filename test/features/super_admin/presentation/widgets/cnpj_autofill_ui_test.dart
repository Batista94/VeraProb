  ...
  group('CnpjAutofillField (CT17)', () {
    ...
    Widget buildWidget() {
      return ProviderScope(
        overrides: [
          cnpjLookupServiceProvider.overrideWithValue(mockService),
        ],
        child: MaterialApp(
          home: Scaffold(
    ...
    testWidgets(
      ...
      await tester.enterText(textField, '45.518.855/0001-47');
      await tester.pump(const Duration(milliseconds: 600)); // Trigger debounce
    ...
    testWidgets(
        'Graceful Degradation: Allows manual editing when API fails',
        (tester) async {
      when(mockService.lookup(any)).thenAnswer((_) async => null);
    ...
    testWidgets(
        'Short CNPJ: Does not trigger lookup for less than 14 digits',
        (tester) async {
      when(mockService.lookup(any)).thenAnswer((_) async => null);
