    ...
    test(
      ...
      () async {
        ...
        expect(
          ...
          throwsA(isA<DomainException>()),
          reason:
              'Contrato closed (mesmo sem nunca ter sido active) deve rejeitar planos',
        );
      },
    );
    ...
    () {
      test(
        ...
        () async {
          ...
          final plan = await b2bPlanHandler.handle(
            DeclareContractualPlanCommand(
              organizationId: 'org-5-1-b2b',
              contractId: contract.id,
          ...
          expect(
            ...
            isTrue,
            reason:
                'ShiftPattern-based plan deve gerar PLAN_DECLARED no ledger',
          );

          // Entrada PLAN_DECLARED aponta para org correta
          final planEntry = b2bLedger.entries.firstWhere(
            (e) => e.type == 'PLAN_DECLARED',
          );
          ...
          expect(
            types2.contains('CONTRACT_ACTIVATED'),
            isTrue,
