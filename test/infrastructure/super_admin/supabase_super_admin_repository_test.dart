...
void main() {
  ...
  group('SupabaseSuperAdminRepository — Edge Proxy', () {
    ...
    test('constructor accepts exactly one SupabaseClient (no service_role field)',
        () {
    ...
    group('getAllTenantHealth', () {
      test('invokes super-admin-proxy with action=list_tenant_health', () async {
        when(
      ...
      test('returns empty list when proxy returns empty data', () async {
        when(
          ...
          (_) async =>
              FunctionResponse(data: <String, dynamic>{'data': []}, status: 200),
        );
    ...
    group('getSystemAuditLog', () {
      test('invokes super-admin-proxy with action=get_audit_log', () async {
        when(
          ...
          (_) async => FunctionResponse(data: <String, dynamic>{'data': []}, status: 200),
        );
      ...
      test('passes limit in params', () async {
        ...
        final params =
            (captured.first as Map<String, dynamic>)['params'] as Map<String, dynamic>;
