...
//
//   Vehicle at kVehicle30mLat → dist_A ≈ 30 m (inside), dist_B ≈ 170 m (outside)
//   Vehicle at kVehicle80mLat → dist_A ≈ 80 m (outside), dist_B ≈ 120 m (outside)
// =============================================================================
...
/// Vehicle position ~30 m north of Stop A → inside 50 m radius of A.
const double kVehicle30mLat = -23.5609297;

/// Vehicle position ~80 m north of Stop A → outside 50 m radius of all stops.
...
void main() {
  ...
  group('classifyMotion', () {
    ...
    test(
      ...
      () {
        ...
        classifier.classifyMotion(
          'v2',
          1.0, // <= slowTrafficThreshold → key inserted
          (kStopALat, kStopALng),
          const [],
          clock.now(),
        );

        // Vehicle is now fast → key must be removed
        final result = classifier.classifyMotion(
          'v2',
        ...
        // timestamp, so duration is 0 → moving (not stopped).
        ...
        // 0 s elapsed since fresh insert < stoppedMinDuration(15 s) → moving
    ...
    test(
      ...
      () {
        // Fresh classifier, v3 never seen. Duration = 0 < stoppedMinDuration → moving.
        final result = classifier.classifyMotion(
          'v3',
    ...
    test(
      ...
      () {
        ...
        final result = classifier.classifyMotion(
          ...
          1.0,
          (kStopALat, kStopALng),
          const [], // no stops → stopped when duration >= threshold
          clock.now(),
        );
        // 20 s >= 15 s stoppedMinDuration → stopped (proves timestamp was NOT reset)
  ...
  group('_findNearestStop via classifyMotion', () {
    ...
    test(
      ...
      () {
        // kVehicle80mLat: dist_A ≈ 80 m, dist_B ≈ 120 m — both > 50 m
  ...
  group('removeKey', () {
    ...
    test(
      ...
      () {
        ...
        // Re-classify: new key inserted at clock.now(), duration = 0 → moving
        final result = classifier.classifyMotion(
          'vR',
          0.0,
          (kStopALat, kStopALng),
  ...
  group('reset', () {
    ...
    test(
      ...
      () {
        ...
        // Both vehicles re-insert a fresh timestamp at clock.now() → duration = 0 → moving
