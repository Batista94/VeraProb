...
void main() {
  ...
  late DisputeSanctionHandler handler;
  ...
  group('DisputeSanctionHandler - RBAC', () {
  ...
  group('DisputeSanctionHandler - Idempotency (INV-24)', () {
    test(
      'throws DomainException if entry is already applied/disputed',
      () async {
    ...
    test('throws DomainException if entry is already disputed', () async {
      final disputedEntry = makePendingEntry().copyWith(
        status: SanctionReviewStatus.disputed,
      );
      await queueRepo.enqueue(
        SanctionReviewQueueEntry(
          id: disputedEntry.id,
          organizationId: disputedEntry.organizationId,
          ledgerEntryId: 'ledger-001',
          setId: 'set-1',
          contractId: 'contract-1',
          verdictEvidence: evidence,
  ...
  group('DisputeSanctionHandler - Tenant isolation (INV-6/INV-22)', () {
    test(
      'throws SovereigntyViolationException if orgId does not match session',
      () async {
  ...
  group('DisputeSanctionHandler - Happy path', () {
    test('appends SANCTION_DISPUTED to ledger', () async {
      await queueRepo.enqueue(makePendingEntry());
      ...
      expect(entries.first.type, 'SANCTION_DISPUTED');
      expect(entries.first.payload['verdict_evidence'], isNotNull);
      expect(entries.first.payload['queue_entry_id'], 'entry-001');
    });

    test('updates queue entry status to disputed', () async {
      await queueRepo.enqueue(makePendingEntry());
      ...
      expect(entry!.status, SanctionReviewStatus.disputed);
    });
  });
}
