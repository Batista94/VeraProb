...
void main() {
  ...
  late DeclinePeerReviewHandler handler;
  ...
  DeclinePeerReviewCommand cmd({required UserRole role}) =>
      DeclinePeerReviewCommand(
        queueEntryId: 'entry-pr',
        declinedByUserId: 'auditor-2',
        actorEmail: 'auditor-2@test.com',
  ...
  setUp(() {
    ...
    handler = DeclinePeerReviewHandler(
  ...
  test('decline reverts to origin and appends PEER_REVIEW_DECLINED', () async {
    await queueRepo.enqueue(peerReviewEntry());
    await handler.handle(cmd(role: UserRole.auditor));
  ...
  test(
    'idemp-double-decline: second decline on a reverted item fails',
    () async {
      await queueRepo.enqueue(peerReviewEntry());
      await handler.handle(cmd(role: UserRole.auditor));
