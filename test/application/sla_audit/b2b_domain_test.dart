...
import 'package:pactaflow/domain/sla_audit/operational_alert_repository.dart';
...
void main() {
  group('Phase 5.10 - In-Memory B2B Domain & Projection Validation', () {
    ...
    test('Scenario 5.5 & 5.6: Determinism and Indempotency (Generation)', () async {
      // Setup Zones
      ...
      expect(sets1.first.setId, sets2.first.setId, reason: 'SHA-256 must be identical');
    ...
    test('Scenario 5.11: Extreme Timezone Offset (+14) normalization', () async {
      ...
      expect(sets.first.scheduledStartTimeUtc.day, 1, reason: 'Must normalize to previous day in UTC');
...
class MockAlertRepo implements OperationalAlertRepository {
  ...
  @override
  Future<List<OperationalAlert>> findByOrganization(String orgId, {String? severity, String? status}) async => [];
  @override
...
class MockRuleRepo implements ContractualRuleRepository {
  @override
  Future<RuleSnapshot> getActiveSnapshotForContract(String orgId, String contractId) async => const RuleSnapshot([]);
  @override
...
class MockContractRepo implements ContractRepository {
  ...
  @override
  Future<List<Contract>> findByOrganization(String orgId, {ContractStatus? status}) async => [];
}
