...
void main() {
  ...
  Future<
    (
      ...
      InMemoryPlanDeclarationRepository,
    )
  >
  makeSetup({
    List<DayOfWeek>? days,
  ...
  group('ShiftProjectionService', () {
    test(
      '5.1 — projectDays returns empty list for non-shift-based plan',
      () async {
    ...
    test(
      '5.3 — determinism: same plan + pattern + date → same setId',
      () async {
    ...
    test(
      '5.4 — idempotency: saveProjectedSets twice does not duplicate',
      () async {
        final (plan, _, _, service, planRepo) = await makeSetup();
    ...
    test(
      '5.5 — zone coordinate snapshot: zone update after projection does not affect existing SET',
      () async {
    ...
    test(
      '5.7 — detectAndAlertGaps raises PROJECTION_GAP for each past missing day',
      () async {
    ...
    test(
      '5.9 — skip days when zone not found returns null (no crash)',
      () async {
        final pattern = ShiftPattern.create(
          index: 0,
          daysOfWeek: [DayOfWeek.monday],
    ...
    test(
      '5.10 — overnight shift: isOvernight is true when departure > arrival',
      () {
        final pattern = ShiftPattern.create(
          index: 0,
          daysOfWeek: [DayOfWeek.monday],
    ...
    test(
      '5.11 — daytime shift: isOvernight is false when departure < arrival',
      () {
        final pattern = ShiftPattern.create(
          index: 0,
          daysOfWeek: [DayOfWeek.monday],
    ...
    test(
      '5.14 — SLAPenalties: new fields have correct defaults and survive JSON roundtrip',
      () {
    ...
    test('5.13 — overnight D+1: projected SET ends on next day in UTC', () async {
      ...
      expect(
        set.scheduledStartTimeUtc.weekday,
        DateTime.tuesday,
      ...
      expect(
        set.scheduledEndTimeUtc.weekday,
        DateTime.tuesday,
