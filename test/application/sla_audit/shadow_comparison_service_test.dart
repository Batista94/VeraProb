...
void main() {
  ...
  group('generateReport — sync wiring', () {
    test('applies pending manual decisions before computing metrics', () async {
  ...
  group('generateReport — tenant isolation', () {
    test('only returns verdicts for the requested organization', () async {
      ...
      // Save a verdict for a different org via direct factory (bypasses repo's
      // org check on save, letting us test the query filter).
      ...
      repo.all; // access to force list evaluation
      final altRepo = InMemoryShadowVerdictRepository()
        ..all; // fresh repo for other org
      await altRepo.save(otherVerdict);
