...
void main() {
  ...
  group('CloseContractHandler', () {
    test('closes a draft contract — appends CONTRACT_CLOSED to ledger', () async {
      ...
      final found = await repository.findById(created.id, organizationId: 'org-1');
    ...
    test('throws DomainException for wrong organization (tenant isolation)', () async {
      final created = await createHandler.handle(CreateContractCommand(
    ...
    test('throws DomainException when closing an already-closed contract', () async {
      final created = await createHandler.handle(CreateContractCommand(
        organizationId: 'org-1',
        name: 'Contract B',
