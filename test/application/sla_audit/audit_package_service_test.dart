...
import 'package:pactaflow/application/sla_audit/audit_package_service.dart';
...
import 'package:pactaflow/infrastructure/sla_audit/in_memory_audit_package_repository.dart';
...
void main() {
  ...
  group('AuditPackageService.createDraftAndSeal', () {
    test('persists TWO rows: draft (row A) then sealed (row B) — D1-Canonical', () async {
    ...
    test('draft and sealed rows have DIFFERENT IDs (D1-Canonical: two-row strategy)', () async {
      await snapshotRepo.save(makeSnapshot(date: DateTime.utc(2026, 3, 1)));
    ...
    test('idempotency guard: returns existing sealed package if already exists', () async {
      await snapshotRepo.save(makeSnapshot(date: DateTime.utc(2026, 3, 1)));
      ...
      expect(auditPackageRepo.count, equals(2)); // still draft + sealed, not 4
    });
    ...
    test('organizationId scoping: packages from different orgs are isolated', () async {
      await snapshotRepo.save(makeSnapshot(date: DateTime.utc(2026, 3, 1)));
  ...
  group('AuditPackageService.supersede', () {
    test('saves superseded row with correct lineage and reason (INV-1)', () async {
      await snapshotRepo.save(makeSnapshot(date: DateTime.utc(2026, 3, 1)));
