...
void main() {
  group('Engine Replay Determinism (INV-7 / INV-12)', () {
    ...
    test('Batching Invariance: Multiple small batches vs Single big batch produce identical final state', () async {
      await seedPlan();
      ...
      final facts = [
        CanonicalFact.reconstitute(
          id: 'F1', organizationId: orgId, assetId: vehicleId, deviceId: 'DEV1', rawPayloadId: 'P1',
          sourceAdapter: 'GPS', receivedAtUtc: t0, gpsTimestamp: t0, lat: -23.5, lng: -46.6,
          integrityFlag: IngestionIntegrityFlag.ok,
        ),
        CanonicalFact.reconstitute(
          id: 'F2', organizationId: orgId, assetId: vehicleId, deviceId: 'DEV1', rawPayloadId: 'P2',
          sourceAdapter: 'GPS', receivedAtUtc: t5, gpsTimestamp: t5, lat: -23.5001, lng: -46.6001,
          integrityFlag: IngestionIntegrityFlag.ok,
        ),
        CanonicalFact.reconstitute(
          id: 'F3', organizationId: orgId, assetId: vehicleId, deviceId: 'DEV1', rawPayloadId: 'P3',
          sourceAdapter: 'GPS', receivedAtUtc: t15, gpsTimestamp: t15, lat: -23.5002, lng: -46.6002,
          integrityFlag: IngestionIntegrityFlag.ok,
        ),
      ];
      ...
      final initialState = ContractualExecutionState.create(
        organizationId: orgId, setId: setId, contractId: contractId, planVersion: 1,
        startLatitude: -23.5, startLongitude: -46.6, startRadiusMeters: 500,
    ...
    test('Temporal Determinism: Late arrival corrections result in consistent final state', () async {
      await seedPlan();
      ...
      final initialState = ContractualExecutionState.create(
        ...
        contractualValue: const Money(1000),
        noShowPenaltyMultiplier: 1.5,
        windowStartUtc: t0, windowEndUtc: DateTime.utc(2026, 3, 1, 11, 0),
      );
      await execRepo.save(initialState);
      ...
      await engine.sweepExpiredObligations(nowUtc: tSweep, organizationId: orgId);

      final stateAtSweep = await execRepo.findBySetId(setId);
      expect(stateAtSweep?.status, equals(ExecutionStatus.noShow), reason: 'Sweep should mark expired pending state as noShow');

      // 2. Late fact arrives (T15) which proves dwell time.
      await pipeline.process([
        CanonicalFact.reconstitute(
          id: 'FLate', organizationId: orgId, assetId: vehicleId, deviceId: 'DEV1', rawPayloadId: 'PL',
          sourceAdapter: 'GPS', receivedAtUtc: DateTime.now().toUtc(), 
          gpsTimestamp: t15, lat: -23.5001, lng: -46.6001, 
          integrityFlag: IngestionIntegrityFlag.lateArrival,
        )
      ], organizationId: orgId);

      final finalState = await execRepo.findBySetId(setId);
      expect(finalState?.status, equals(ExecutionStatus.executed), reason: 'Late arrival MUST correct noShow state to executed');
