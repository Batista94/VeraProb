...
void main() async {
  ...
  group('RLS Sovereignty Audit (Red Team Expansion)', skip: !isRunning ? 'Supabase local instance not detected' : null, () {
    ...
    setUpAll(() async {
      ...
      await _ensureOrg(adminClient, id: _orgAId, name: 'RedTeam — Target Alpha');
      await _ensureOrg(adminClient, id: _orgBId, name: 'RedTeam — Target Beta');

      final userAId = await _ensureUser(adminClient, email: _userAEmail, password: _testPassword);
      final userBId = await _ensureUser(adminClient, email: _userBEmail, password: _testPassword);
      final operatorId = await _ensureUser(adminClient, email: _operatorEmail, password: _testPassword);

      await _assignRole(adminClient, userId: userAId, orgId: _orgAId, role: 'TENANT_ADMIN');
      await _assignRole(adminClient, userId: userBId, orgId: _orgBId, role: 'TENANT_ADMIN');
      await _assignRole(adminClient, userId: operatorId, orgId: _orgAId, role: 'OPERATOR');
      ...
      await adminClient.from('contractual_evaluation_traces').insert({
        ...
        'decisions_jsonb': [{'outcome': 'SANCTION_RECOMMENDED', 'reason': 'Late Arrival Breach'}]
      });
      ...
      await adminClient.from('operational_alerts').insert({
        ...
        'context': {'message': 'Unauthorized bypass attempt detected (simulated)', 'attacker_mock': 'RedTeam'}
      });
    ...
    // ── Sanity Check ────────────────────────────────────────────────────────
    test('SANITY — Availability: Tenant B can see their own ledger', () async {
       final res = await tenantBClient.from('sla_audit_ledger_v2').select('id');
       expect(res, isNotEmpty, reason: 'Availability Failure: Tenant B blocked from their own data');
    });
    ...
    // ═════════════════════════════════════════════════════════════════════════
    test('AUDIT 1 — Partitioned Ledger Sovereignty: Org A cannot read Org B ledger_v2', () async {
      final res = await tenantAClient
          .from('sla_audit_ledger_v2')
          .select()
          .eq('organization_id', _orgBId);
      
      expect(res, isEmpty, reason: 'RLS breach: Tenant Alpha accessed Tenant Beta ledger entries');
    });

    // ═════════════════════════════════════════════════════════════════════════
    // AUDIT 2: Explainability Isolation (INV-7)
    // ═════════════════════════════════════════════════════════════════════════
    test('AUDIT 2 — Trace Explainability Isolation: Org A cannot read Org B evaluation traces', () async {
      final res = await tenantAClient
          .from('contractual_evaluation_traces')
          .select()
          .eq('organization_id', _orgBId);
      
      expect(res, isEmpty, reason: 'RLS breach: Tenant Alpha accessed Tenant Beta evaluation traces');
    });

    test('AUDIT 3 — Alert Sovereignty: Org A cannot read Org B operational alerts', () async {
      final res = await tenantAClient
          .from('operational_alerts')
          .select()
          .eq('organization_id', _orgBId);

      expect(res, isEmpty, reason: 'RLS breach: Tenant Alpha accessed Tenant Beta operational alerts');
    });

    // ═════════════════════════════════════════════════════════════════════════
    // AUDIT 4: LGPD PII Masking (LGPD Compliance)
    // ═════════════════════════════════════════════════════════════════════════
    test('AUDIT 4 — LGPD PII Masking: OPERATOR role sees masked CNPJ and Email', () async {
    ...
    test('AUDIT 5 — LGPD PII Access: TENANT_ADMIN sees full PII details', () async {
      final res = await tenantAClient
          .from('contractors_view')
          .select('tax_id, primary_email')
          .limit(1)
    ...
    // ═════════════════════════════════════════════════════════════════════════
    test('AUDIT 6 — Sovereignty Injection: Rejected when Org A attempts to insert Ledger for Org B', () async {
      ...
      await expectLater(
        ...
        reason: 'Security breach: RLS WITH CHECK failed to reject mismatched organization_id'
      );
    });
    ...
    // ═════════════════════════════════════════════════════════════════════════
    test('AUDIT 7 — Forensic Immutability: Even service_role cannot DELETE from Ledger V2', () async {
      ...
      await expectLater(
        () async => adminClient.from('sla_audit_ledger_v2').delete().eq('id', ledgerId),
        throwsA(isA<PostgrestException>()),
        reason: 'Forensic Breach: Immutability trigger failed to block DELETE for ledger_v2'
      );
    });
  });
}
...
Future<String> _ensureUser(SupabaseClient admin, {required String email, required String password}) async {
  ...
  final listRes = await http.get(
    Uri.parse('${PostgresTestConfig.supabaseUrl}/auth/v1/admin/users?email=$email'),
    headers: {
      'apikey': PostgresTestConfig.serviceRoleKey,
      'Authorization': 'Bearer ${PostgresTestConfig.serviceRoleKey}',
    },
  );
  final users = (jsonDecode(listRes.body) as Map<String, dynamic>)['users'] as List;
  return users.first['id'] as String;
}

Future<void> _ensureOrg(SupabaseClient admin, {required String id, required String name}) async {
...
Future<void> _assignRole(SupabaseClient admin, {required String userId, required String orgId, required String role}) async {
...
Future<SupabaseClient> _signIn(String email, String password) async {
  final client = SupabaseClient(PostgresTestConfig.supabaseUrl, PostgresTestConfig.supabaseAnonKey);
