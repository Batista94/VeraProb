...
void main() {
  ...
  group('Feature: superadmin-org-management-e2e-tests, '
    ...
    for (var i = 0; i < iterations; i++) {
      ...
      test('iter $i [PREDICATE]: Archived org ($numAdmins admins) → '
          'all admins !active ∧ banned (both layers)', () async {
    ...
    for (var i = 0; i < iterations; i++) {
      ...
      test('iter $i [CONTRAPOSITIVE]: Org with active admins ($numAdmins) → '
          'org status ≠ ARCHIVED', () async {
        // `skip:` is evaluated at collection time, before setUpAll sets
        // supabaseAvailable — guard at runtime instead.
        if (!supabaseAvailable) {
          markTestSkipped(
