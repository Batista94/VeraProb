...
-- Setup
INSERT INTO organizations (id, name) VALUES ('c0000000-0000-0000-0000-000000000001', 'Org C') ON CONFLICT (id) DO NOTHING;
...
SELECT lives_ok(
    $$ SELECT webhook_delivery_fail('c3000000-0000-0000-0000-000000000001', 'c0000000-0000-0000-0000-000000000001', 'HTTP_500') $$,
...
SELECT lives_ok(
    $$ SELECT webhook_delivery_fail('c3000000-0000-0000-0000-000000000002', 'c0000000-0000-0000-0000-000000000001', 'HTTP_500') $$,
...
-- Case 3 (INV-1): a mismatched org id cannot mutate another tenant's row.
INSERT INTO webhook_delivery_logs (id, organization_id, endpoint_id, ledger_entry_id, event_type, payload, status, attempt_count)
VALUES ('c3000000-0000-0000-0000-000000000003', 'c0000000-0000-0000-0000-000000000001', 'c1000000-0000-0000-0000-000000000001', 'c2000000-0000-0000-0000-000000000001', 'DISPUTE_ACCEPTED', '{}', 'DELIVERING', 2);

SELECT lives_ok(
    $$ SELECT webhook_delivery_fail('c3000000-0000-0000-0000-000000000003', '00000000-0000-0000-0000-000000000099', 'HTTP_500') $$,
    'Wrong-org call is a silent no-op (row untouched)'
);

SELECT results_eq(
    $$ SELECT status, attempt_count FROM webhook_delivery_logs WHERE id = 'c3000000-0000-0000-0000-000000000003' $$,
    $$ VALUES ('DELIVERING'::webhook_delivery_status, 2) $$,
    'INV-1: row unchanged when p_org_id does not match'
);
