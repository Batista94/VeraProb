...
-- =============================================================================
-- Migration: Ledger SSOT — autonomous closer writes sla_audit_ledger_v2 (PR4)
-- Freezes adding HASH partitions until measured hotspots exist.
-- Depends on 20260923000002 (SYSTEM_AUTO_CLOSE in ledger_event_type).
-- Invariants: INV-3, INV-6. INV-DB: CREATE OR REPLACE only.
-- =============================================================================

SET client_min_messages TO 'WARNING';

COMMENT ON TABLE public.sla_audit_ledger_v2 IS
  'Forensic ledger SSOT. HASH p0-p3 frozen — do not add partitions without scale evidence (ponytail PR4).';

COMMENT ON TABLE public.sla_audit_ledger IS
  'LEGACY v1 — stop new writes. Readers must migrate to sla_audit_ledger_v2 (20260923000003).';
...
BEGIN
  ...
  -- Step 9: INV-3 audit on ledger SSOT (v2). actor_* live in payload (no v1 cols).
...
COMMENT ON FUNCTION public.check_and_close_execution_autonomously IS
  'GPS autonomous closer. Ledger writes → sla_audit_ledger_v2 (PR4 SSOT). INV-1,3,6,15,26.';
