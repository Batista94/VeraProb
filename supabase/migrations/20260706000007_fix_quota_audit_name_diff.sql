-- ── Migration 20260706000007 ─────────────────────────────────────────────────
-- Fix: super_admin_update_organization_quota audit log
--
-- Problems fixed:
--   1. event_type hardcoded 'QUOTA_CHANGE' even for name-only changes.
--      Now derives 'OPERATIONAL_PARAM_CHANGE' when quota fields are unchanged.
--   2. before/after payload omitted trade_name / legal_name — forensic diff
--      was impossible for cadastral changes (INV-3, INV-21).
--   3. actor_type was NULL — now set to 'HUMAN'.
--
-- Dart double-write removed separately (handler Step 6 deleted).
-- Signature unchanged → CREATE OR REPLACE safe.
-- ─────────────────────────────────────────────────────────────────────────────
