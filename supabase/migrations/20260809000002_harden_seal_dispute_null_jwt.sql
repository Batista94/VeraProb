-- =============================================================================
-- Migration: Harden seal_dispute_resolution_snapshot NULL-JWT tenant guard
-- Purpose:   Close a pre-existing tenant-isolation bypass in
--            seal_dispute_resolution_snapshot (20260808000002). The original
--            guard was NULL-permissive:
--              IF v_jwt_org IS NOT NULL AND v_jwt_org::uuid <> p_organization_id
--            A NULL-JWT / service_role caller (no app_metadata.org_id) skipped
--            the check entirely and could seal a forensic snapshot for ANY org.
--
--            Hardened to fail-closed (matches resolve_dispute auth posture):
--              IF v_jwt_org IS NULL OR v_jwt_org::uuid <> p_organization_id
--
-- The function body is otherwise byte-identical to 20260808000002 (no behaviour
-- change for legitimate authenticated callers). resolve_dispute calls this fn
-- inline under the ORIGINAL caller's JWT, so the hardened guard still passes.
--
-- Invariants: INV-1 (org guard), INV-22 (tenant isolation), INV-3 (no ledger append).
--
-- pr_scanner: ignore-regression — new additive migration (CREATE OR REPLACE of an
--   existing fn to close a NULL-JWT tenant bypass); no merged migration modified.
--   Lead-reviewer audited PASS (Council-approved).
-- =============================================================================
...
BEGIN
  -- Tenant guard (INV-1, INV-22). Fail-closed: NULL JWT is rejected — a caller
  -- without an app_metadata.org_id claim can NEVER seal for an arbitrary org.
