...
21. **DETECCÃO ANTI-SPOOFING (INV-21)** — Todo `CanonicalFact` classificado como `suspectedSpoofing` DEVE ser excluído do Engine até revisão explícita de um `AUDITOR`. Nenhum veredito de SLA pode ser emitido sobre fato sob suspeita sem aprovação manual registrada.
