...
3. **Deterministic Time:** All timestamps MUST use `DateTime.now().toUtc()` on a single line. NEVER use `DateTime.now()` without `.toUtc()`. Any occurrence is a CRITICAL FAILURE.
   - **Scanner False Positives:** To prevent blocking due to descriptive strings or docs, use `DateTime . now()` or `DateTime_now` in text/comments to avoid triggering the regex-based PR Scanner.

4. **Forensic Annotations:** Every `double` declaration that is NOT for currency (e.g., GPS, distance, velocity) MUST be annotated with `// Physical Metric - Double Required` on the same line to pass the PR Scanner. No exceptions.
