...
void main() {
  ...
  group('Cenários Adversos e Condições de Corrida', () {
    ...
    testWidgets(
      '8.1 Consistência ao arquivar org durante edição concorrente '
      '(race condition)',
      (tester) async {
    ...
    testWidgets(
      '8.2 Exibição de erro e retry ao interromper rede durante '
      'arquivamento',
      (tester) async {
        if (!supabaseAvailable) {
          markTestSkipped('Supabase local não disponível.');
        ...
        final retryArchiveText = find.widgetWithText(
          FilledButton,
          'Arquivar',
        );
        ...
        final canRetry = retryArchiveButton.evaluate().isNotEmpty ||
    ...
    testWidgets(
      '8.3 Idempotência de double-click em desativação de admin '
      '(apenas 1 operação processada)',
      (tester) async {
        if (!supabaseAvailable) {
          markTestSkipped('Supabase local não disponível.');
          return;
        ...
        expect(
          ...
          findsAtLeast(1),
          reason:
              'O botão "Inativar Usuário" deve estar visível (Req 8.3)',
        );
        ...
        try {
          ...
          final logsForAdmin = auditLogs.where(
            (log) {
          ...
          expect(
            ...
            isFalse,
            reason:
                'O admin deve estar desativado após a operação (Req 8.3)',
          );
        } finally {
    ...
    testWidgets(
      '8.4 Navegação para fora durante operação em andamento '
      '(complete ou cancel limpo)',
      (tester) async {
        if (!supabaseAvailable) {
          markTestSkipped('Supabase local não disponível.');
          return;
        ...
        expect(
          ...
          isTrue,
          reason:
              'O modal deve ser fechado ao navegar para fora (Req 8.4)',
        );
    ...
    testWidgets(
      '8.5 Redirecionamento para login ao expirar token — dados '
      ...
      (tester) async {
        ...
        final orgNameVisible = find.textContaining(
          testOrgTokenExpiry.orgName,
        );
        expect(
          orgNameVisible,
          findsAtLeast(1),
          reason:
              'O nome da org deve estar visível antes da expiração do token',
        );
        ...
        final isOnLoginScreen = loginFields.evaluate().isNotEmpty &&
    ...
    testWidgets(
      '8.6 Mensagem apropriada ao revogar convite já aceito '
      '(race condition)',
      (tester) async {
        if (!supabaseAvailable) {
          markTestSkipped('Supabase local não disponível.');
    ...
    testWidgets(
      '9.6 Erro de CNPJ duplicado: rejeição ao criar org com CNPJ '
      'já existente',
      (tester) async {
        if (!supabaseAvailable) {
          markTestSkipped('Supabase local não disponível.');
        ...
        final submitConfirmar = find.widgetWithText(
          FilledButton,
          'Confirmar',
        );
        ...
        try {
          ...
          expect(
            ...
            reason:
                'A única org com este CNPJ deve ser a original (Req 9.6)',
          );
        } finally {
          await verifyClient.dispose();
        }
