...
class DeclareContractualPlanHandler with IdempotentHandlerMixin {
  ...
  Future<PlanDeclaration> handle(DeclareContractualPlanCommand command) async {
    return await executeWithIdempotency<PlanDeclaration>(
      ...
      reloadEntity: (dto) => _repository.findByContract(
        command.contractId,
        organizationId: command.organizationId,
      ).then((list) => list.firstWhere(
        (p) => p.planVersion == dto['version'],
      )),
      ...
      recoverIfAlreadyCompleted: () async {
        ...
        return existing.where((p) => p.originalFileHash == command.originalFileHash).firstOrNull;
      },
    );
  }

  Future<PlanDeclaration> _execute(DeclareContractualPlanCommand command) async {
    ...
    final vehicleCount = await _vehicleRepository.countActiveByOrganization(command.organizationId);
    if (vehicleCount == 0) {
      throw const DomainException('No active vehicles found for this organization');
    }

    // 2.2 Operational zone gate (INV-18)
    final zones = await _zoneRepository.findByOrganization(command.organizationId);
    if (zones.isEmpty) {
      throw const DomainException('No operational zones configured for this organization');
    }
    ...
    final plan = command.shiftPatterns.isNotEmpty
        ? PlanDeclaration.createWithShiftPatterns(
            ...
            ruleSnapshot: RuleSnapshot(const []), // Simplified for MVP
        ...
        : PlanDeclaration.create(
            ...
            declaredAtUtc: command.declaredAtUtc,
            ruleSnapshot: RuleSnapshot(const []), // Simplified for MVP
            services: command.services.map((input) => ContractualServiceExecution.create(
              contractId: command.contractId,
