...
## 8. FLUTTER WEB WASM — ASYNC CONTEXT IN DIALOGS (WASM-CONTEXT-LEAK, CT02)
...
## 10. NO IIFE IN WIDGET TREES (IIFE-UI-SMELL)

**Rule:** Never use Immediately Invoked Function Expressions (IIFEs) like `() { ... }()` inside `build()` methods or Riverpod `switch` expressions. Avoid nested ternary operators (`a ? b : c ? d : e`).

**Why:** IIFEs and nested ternaries bloat the widget tree, break Hot Reload optimizations, make code unreadable, and violate clean code separation of concerns. They are a common "lazy" hack generated by AI to avoid extracting methods.

**How to apply:** Always extract complex conditional logic or block statements into separate helper methods (`Widget _buildInvitations(...)`). Keep `build()` highly declarative.
